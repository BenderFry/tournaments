...
def update_knockout_stage():
    ...
    loosers = get_looser_tabelle()
    # Each entry is (group, player_name, info)
    if len(loosers) >= 4:
        ...
        # VF1: 2nd place A/B vs 3rd place A/B
    knockout_matches[1]['teams'] = [[g(A, 2), g(B, 2)], [g(A, 3), g(B, 3)]]

    # VF2: 2nd place C/D vs winner AF1
    ...
    knockout_matches[2]['teams'] = [[g(C, 2), g(D, 2)], get_winner(knockout_matches[0])]
    # HF1: winner VF1 vs winner VF2
    knockout_matches[3]['teams'] = [get_winner(knockout_matches[1]), get_winner(knockout_matches[2])]
    # HF2: winner VF1 vs winner VF2 (if you have additional matches)
    knockout_matches[4]['teams'] = [get_winner(knockout_matches[1]), get_winner(knockout_matches[2])]
